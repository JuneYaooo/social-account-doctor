...
账号 URL → 平台识别 → 提示该调哪个 TikHub REST API 工具拿账号信息 + 笔记列表。

不直接发起网络 API 请求；本脚本只做：
1. 解析 URL，识别平台（仅支持小红书 / 抖音 / 快手）
2. 提取 user_id / sec_user_id / username（平台不同字段不同）
3. 输出 JSON：告诉 caller platform / user_id / 要调哪个 TikHub REST API 工具

caller 拿到 JSON 后，自己去调对应 TikHub REST API，再把账号信息 + 笔记列表回填给诊断流程。
...
PLATFORM_RULES = [
    {
        ...
        "tools": {
            "extract_share": "tikhub xiaohongshu xiaohongshu_app_v2_get_user_info",
            ...
            "note_detail": "tikhub xiaohongshu xiaohongshu_web_v3_fetch_note_detail",
            "video_note_detail": "tikhub xiaohongshu xiaohongshu_app_v2_get_video_note_detail",
            "note_comments": "tikhub xiaohongshu xiaohongshu_app_v2_get_note_comments",
            "search_notes": "tikhub xiaohongshu xiaohongshu_app_v2_search_notes",
            "search_users": "tikhub xiaohongshu xiaohongshu_app_v2_search_users",
            "hot_list": "tikhub xiaohongshu xiaohongshu_web_v3_fetch_hot_list",
        },
        "needs_share_resolve": True,
        "share_param": "share_text",
        "id_field": "user_id",
        "extra_param_note": "小红书 App V2 账号接口可直接传 user_id 或 share_text；短链优先把完整分享文本传给 user_info",
    },
    {
        "platform": "douyin",
        ...
        "tools": {
            ...
            "user_info_by_sec": "tikhub douyin douyin_app_v3_handler_user_profile",
            ...
            "search_users": "tikhub douyin douyin_search_fetch_user_search_v2",
            "search_videos": "tikhub douyin douyin_search_fetch_video_search_v2",
    ...
    },
    {
        ...
        "tools": {
            ...
            "video_comments": "tikhub kuaishou kuaishou_app_fetch_video_comment",
...
def main():
    ...
    if needs_resolve and "extract_share" in rule["tools"]:
        next_step = (
            f"短链/无 ID — 先调 REST API 端点 `{rule['tools']['extract_share']}` "
            f"(传 {rule['share_param']}={url}) 解析出 {rule['id_field']}"
        )
    elif primary_id:
        primary_tool = None
        ...
        next_step = (
            ...
            f"直接调 REST API 端点 `{primary_tool}` 拿账号信息"
        )
    else:
        next_step = (
    ...
    output = {
        ...
        "next_step": next_step,
        "playbook": (
            "1. caller 按 next_step 调 TikHub REST API 拿账号基本信息（粉丝数 / 简介 / 认证）\n"
