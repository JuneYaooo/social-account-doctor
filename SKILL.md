...
完整复用 `references/diagnostic-mode.md` 的三层诊断，不能另造一套脱离对标的电商评分：
...
5. 输出“账号问题 → 对标证据 → 下一轮内容动作”；没有有效对标时只能交付部分诊断。

### commerce-review：视频链接/本地视频 + 对标审查

如果输入是平台视频链接，先按 §7 的通用 `crack` 工具表解析分享链接、获取详情和媒体地址，下载到临时目录；输入已经是本地文件时直接进入多模态。媒体拿不到时不得只读标题和互动数据冒充完整内容分析。
