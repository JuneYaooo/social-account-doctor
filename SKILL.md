...
**副产品**：每跑一次 crack，钩子自动追加到 `./assets/hooks-{platform}.md`。**不进主流程**，但跑多了形成你的弹药库。
...
**副产品**：每条钩子句自动追加到 `./assets/hooks-{platform}.md`，按情绪锚点 / 反差点分类（追加，不覆盖）。
...
8. **副产品 hooks-{platform}.md 是追加不是覆盖**。这是用户长期资产，不能丢。
