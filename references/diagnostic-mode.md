...
平均值容易被单条异常爆款拉高。筛选账号内 top/bottom 时同时计算 `账号相对赢家指数 = 作品互动量 / 同一时间窗口作品互动中位数`；样本少于 10 条时只展示原始值，不给稳定公式标签。绝对播放量用于看规模，账号相对指数用于找“这个账号真正重复做对了什么”。
...
六维拆解完成后，用我方 top/bottom 和对标 top 样本归纳可重复公式：

1. 按内容赛道/场景整理发布占比、账号相对赢家和代表作品。
2. 同一钩子、结构、视觉证明、CTA 或评论需求至少在 2 条赢家中重复，且没有同样高频出现在 bottom，才标成“已验证公式”。
3. 只出现 1 次的写“单条假设”，不能升级成账号定位结论。
4. 每个建议写必备素材、生产门槛和主要风险，区分“学机制”与“抄表达”。
5. 输出 `继续保持 / 从对标补齐 / 不要照抄`，下一轮只替换一个变量并指定复盘指标。
...
如果账号以带货内容为主，继续读取 `references/commerce-workflow.md` 和 `references/commerce-analysis-template.md`。在 Layer 2 保证对标账号同品类、同内容形态，在 Layer 3 追加商品露出、利益点顺序、证明动作、信任点、CTA、商品一致性和合规风险；最后从 top/bottom 样本综合账号重复公式、复刻门槛与下一轮单变量实验。没有商家/创作者后台数据时，不根据公开互动推断商品点击率、成交率、GMV 或佣金。
...
### 5.1 账号可重复公式（跨样本综合）
| 内容赛道/公式 | 我方 top 命中 | 我方 bottom 命中 | 对标证据 | 结论 | 必备素材/门槛 |
|---|---|---|---|---|---|
| | | | | 已验证 / 单条假设 / 不成立 | |

| 动作类型 | 内容 |
|---|---|
| 继续保持 | 我方已有数据和多条内容证据支持的动作 |
| 从对标补齐 | 同类、相近体量对标重复有效而我方缺失的动作 |
| 不要照抄 | 依赖大号体量、团队制作、品牌资源或不适合我方定位的动作 |

下一轮单变量测试：保留已验证公式，只替换 `[变量]`；观察 `[指标]`；复盘窗口 `[时间/样本数]`。
