...
> 给它本地带货视频或商品详情/SKU/资质材料，它会生成可溯源的事实卡、口播脚本和分镜。
...
- **看得懂带货素材** —— 分析本地视频的商品露出、利益点和转化结构；基于用户商品资料出 15s/30s/60s 口播和分镜
...
| 「分析这个带货视频」+ 本地视频文件 | 4 维钩子 + 商品露出 + 利益点顺序 + 转化结构 + 风险 |
...
安装器支持 `--target claude|codex|cursor|openclaw`；完整命令见 `docs/install.md`。电商流程不读取在线商品页或平台视频，只使用用户上传材料，不会把未核验信息写成商品事实。

> 电商边界：不支持只给商品链接自动分析、在线找带货对标、自动选品、趋势监控和销量/GMV 推断。
