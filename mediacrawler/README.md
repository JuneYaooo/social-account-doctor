...
登录的是**用户自己的账号**，抓太密会触发平台风控（验证码、限流甚至封号）。mc 的默认参数已经是保守档（并发 1、评论仅 detail/creator 默认抓且每条 ≤10、单次几十条），在此之上还有五道机制保护：

- **同平台 30 分钟冷却**：同一平台两次抓取间隔不足会直接拒绝，报错里提示合并关键词或用 `--force` 覆盖
- **失败退避 10 分钟**：只要浏览器拉起过（无论成败），失败后也会进入短冷却——失败往往发生在风控敏感期，立即重试只会加压
- **全机单实例锁**：任何时刻只允许一个抓取进程（**跨平台并行也算并行**）——并行会抢浏览器登录 profile（可能损坏登录态、要重新扫码），mc 直接拒绝；崩溃残留的锁 2 小时后自动失效
- **体量硬上限**：单次 search ≤ 3 词、detail ≤ 5 条、creator ≤ 2 个账号，超了直接拒绝（`MC_MAX_KEYWORDS` / `MC_MAX_DETAIL_IDS` / `MC_MAX_CREATOR_IDS` 可调，自担风险）
- **请求间隔随机抖动**：上游固定 2s 间隔是异常检测最经典的机器特征，`mc --setup` 自动打补丁改成 3-6s 随机停顿（快手上游自带抖动，无需处理）；`mc --status` 的 `pacing_patched` 字段可查
